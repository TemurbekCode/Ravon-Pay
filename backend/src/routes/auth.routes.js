...
function findByIdentifier(identifier) {
  ...
  return db.prepare('SELECT * FROM users WHERE LOWER(email) = ? OR phone = ?').get(value, identifier?.trim());
}

router.post('/register', bruteForceLimiter, (req, res) => {
  ...
  const existing = db.prepare('SELECT id FROM users WHERE LOWER(email) = ?').get(email.trim().toLowerCase());
  if (existing) {
  ...
  db.prepare(`INSERT INTO users (id, email, phone, password_hash, full_name, company_name, account_type, profiles, role, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
  ...
  seedEmptyWalletAndBusiness(userId, fullName, email.trim().toLowerCase());

  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(userId);
  res.json({ accessToken: signToken(userId), user: serializeUser(row) });
});

router.post('/login', bruteForceLimiter, (req, res) => {
...
router.post('/google', bruteForceLimiter, async (req, res) => {
  ...
  if (!row) {
    ...
    seedEmptyWalletAndBusiness(userId, profile.name || 'Google User', email);
...
router.post('/switch-profile', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET account_type = ? WHERE id = ?').run(accountType, req.userId);
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.userId);
  res.json({ user: serializeUser(row) });
});

router.post('/activate-profile', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET profiles = ?, account_type = ?, company_name = ? WHERE id = ?')
...
router.patch('/me', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET full_name = COALESCE(?, full_name), phone = COALESCE(?, phone) WHERE id = ?')
    .run(fullName || null, phone ?? null, req.userId);
  const row = db.prepare('SELECT * FROM users WHERE id = ?').get(req.userId);
  res.json({ user: serializeUser(row) });
});

router.post('/change-password', requireAuth, bruteForceLimiter, (req, res) => {
