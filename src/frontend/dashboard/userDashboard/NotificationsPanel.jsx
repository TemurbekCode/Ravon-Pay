...
const ICONS = {
  ...
  email_reminder: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m22 6-10 7L2 6" /></svg>,
};

// Bildirishnomalar paneli — pul yechilsa/tushsa yoki RavonPay yangilik chiqarsa,
// shu yerda ko'rinadi. Ochilganda barchasi o'qilgan deb belgilanadi. "email_reminder"
// turidagi bildirishnoma bosilsa, Sozlamalar bo'limiga o'tkazadi (email qo'shish
// uchun) — biznes tomonidagi "bizinfo" bildirishnomasi bilan bir xil naqsh.
export default function NotificationsPanel({ show, onClose, notifications, onMarkAllRead, onOpenSettings, t }) {
  ...
  const handleClick = (n) => {
    if (n.kind === 'email_reminder' && onOpenSettings) {
      onOpenSettings();
      onClose();
    }
  };
      ...
      <div className="modal notif-modal">
        ...
        {notifications.length === 0 ? (
          ...
          <div className="notif-list">
            {notifications.map((n) => (
              <div
                className={`notif-item ${n.read ? '' : 'unread'} ${n.kind === 'email_reminder' ? 'clickable' : ''}`}
                key={n.id}
