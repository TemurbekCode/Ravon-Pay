...
export default function Utilities() {
  ...
  const { balance, transactions, payUtility } = useBusiness();
  ...
  const recentAccounts = useMemo(() => {
    ...
    for (const tx of transactions) {
      if (tx.name !== "Kommunal to'lov") continue;
      const acc = tx.email;
  ...
  return (
    <>
      <div className="page-head reveal"><div><h1>{t('page.utilities')}</h1><p>{t('util.sub')}</p></div></div>
      ...
      {active && (
        <div className="panel reveal">
          ...
          {receipt ? (
            <div className="util-receipt" style={{ maxWidth: 420 }}>
              ...
              <button type="button" className="btn-new" style={{ width: '100%', justifyContent: 'center' }} onClick={() => select(active)}>{t('util.receiptAgain')}</button>
            </div>
          ) : (
            <>
              <div className="available-balance" style={{ textAlign: 'left', marginBottom: 18 }}>{t('send.available')}: <strong>{formatCurrency(balance.available)} so'm</strong></div>
