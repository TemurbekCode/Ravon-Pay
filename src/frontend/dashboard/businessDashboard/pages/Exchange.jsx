...
export default function Exchange() {
  ...
  return (
    <>
      <div className="page-head reveal"><div><h1>{t('page.exchange')}</h1></div></div>

      <div className="kpi-row">
        {RATES.map((r) => (
          <div className="kpi-sm reveal" key={r.code}>
            <div className="t"><span className="ic" style={{ fontSize: 18, background: 'none' }}>{r.flag}</span><span className={`kpi-pct ${r.pct >= 0 ? 'up' : 'down'}`}>{r.pct >= 0 ? '+' : ''}{r.pct}%</span></div>
            <div className="n">{r.sell.toLocaleString('ru-RU')}</div>
            <div className="c">1 {r.code} → so'm</div>
          </div>
        ))}
      </div>
      ...
      <div className="panel reveal">
        <div className="panel-head">
          <div><div className="panel-title">{t('exchange.conv')}</div><div className="panel-sub">{t('exchange.convSub')}</div></div>
        </div>
